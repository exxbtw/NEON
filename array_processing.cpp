...
int64_t process_array_scalar(const int32_t* data, size_t n) {
    ...
    for (size_t i = 0; i < n; ++i) {
        int32_t val = data[i];
        if      (val > 0) sum += val;
        else if (val < 0) sum -= val;  //модуль отриц
    }
    return sum;
}
...
int64_t process_array_neon(const int32_t* data, size_t n) {
    ...
    //Основной цикл: 8 элем за итерацию (два NEON-регистра)
    for (; i + 7 < n; i += 8) {
        //Предзагрузка след блока в кэш
        __builtin_prefetch(data + i + 16, 0, 1);

        //Множеств загрузка (SIMD): vld1q_s32 = 128 бит за раз
        int32x4_t vec0 = vld1q_s32(data + i);
        int32x4_t vec1 = vld1q_s32(data + i + 4);

        //Безветвевое вычисление модуля (баррельный шифтер):
        ...
        //Условное выполнение без ветвлений (маски):
        ...
        //Полож - оставляем vec, отриц - abs, нули - 0
    ...
    //Горизонтальное сложение: объединяем acc0 и acc1,
...
//Ассемблерная вставка: демонстрация баррельного шифтера
...
int main() {
    ...
    //Выравнивание памяти: alignas(16) / aligned_alloc
